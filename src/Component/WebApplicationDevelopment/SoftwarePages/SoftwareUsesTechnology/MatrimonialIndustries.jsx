import React from "react";

const MatrimonialIndustries = () => {
  return (
    <div className="m-0">
      <div className="dashboard-main-box">
        <h4 className="Key-feature-heading mb-5" style={{ fontSize: '40px', textAlign: "center" }}>Software Uses Industries</h4>
        <div className="dashboard-container">
          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/matrimoniyaintriweb.webp" alt="Website Integration" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Website Integration
              </h4>
              <p>
                Website Integration simplifies partner search and profile
                creation, offering vast databases for individuals to find
                suitable matches efficiently.
              </p>
            </div>
          </div>

          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/support.webp" alt="Matrimonial Agency Services" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Matrimonial Agency Services
              </h4>
              <p>
                Matrimonial Agency Services utilize software tools to offer
                personalized matchmaking services, ensuring clients find
                compatible life partners efficiently.
              </p>
            </div>
          </div>

          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/matriservies.webp" alt="Agency Services" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Agency Services
              </h4>
              <p>
                Agency Services provide personalized matchmaking expertise,
                guiding clients through the process and offering tailored
                recommendations.
              </p>
            </div>
          </div>

          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/manrimobapi.webp" alt="Mobile Integration" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Mobile Integration
              </h4>
              <p>
                Mobile Integration extends matchmaking services to smartphones,
                allowing users to access profiles and connect with potential
                partners on-the-g
              </p>
            </div>
          </div>
        </div>
        <div className="dashboard-container">
          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/matnrirngage.webp" alt="Community Engagement" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Community Engagement
              </h4>
              <p>
                Community Engagement fosters connections within specific
                communities, facilitating introductions and meetings among
                individuals seeking partners.
              </p>
            </div>
          </div>

          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/mantriwending.webp" alt="Wedding Assistance" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Wedding Assistance
              </h4>
              <p>
                Wedding Assistance includes matchmaking features, ensuring
                couples share similar values and lifestyles for a harmonious
                union.
              </p>
            </div>
          </div>
          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/mantriref.webp" alt="Referral Networks" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Referral Networks
              </h4>
              <p>
                Referral Networks leverage trusted connections, where relatives
                refer potential matches, enhancing compatibility and trust in
                the process.
              </p>
            </div>
          </div>

          <div className="dashboard-container-box dashboard-boxes">
            <div className="key-feature-heading">
              <img src="./images/compliance-document.webp" alt="Event Integration" />

              <h4 className="software-feature-headings d-flex flex-column justify-center align-center">
                Event Integration
              </h4>
              <p>
                Event Integration transforms gatherings into matchmaking
                opportunities, fostering interactions and connections in a
                relaxed atmosphere.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default MatrimonialIndustries;
